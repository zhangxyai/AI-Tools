...
SGLANG_CONFIG_PREFIX=/config/$MODEL_NAME/$WORKER_TYPE
SGLANG_CONFIG_FILE_PATH=$SGLANG_CONFIG_PREFIX/$CONFIG_FILENAME
ENV_CONFIG_FILE_PATH=$SGLANG_CONFIG_PREFIX/$ENV_FILENAME

if [ $RUN_MODE == "test" ]; then
    docker run -itd --privileged=true --gpus all --ipc host --network host --name ${DOCKER_CONTAINER_NAME}-test \
        -v /lib/modules:/lib/modules \
        -v /dev:/dev \
        -v $HOME/.cache:/root/.cache \
        -v $MODEL_WEIGHT_PATH:/models \
        -v $SCRIPT_DIR/config:/config \
        -v $SCRIPT_DIR/script:/script \
        $DOCKER_IMAGE
else
    docker run -d --restart unless-stopped --privileged=true --gpus all --ipc host --network host --name ${DOCKER_CONTAINER_NAME} \
        -v /lib/modules:/lib/modules \
        -v /dev:/dev \
        -v $HOME/.cache:/root/.cache \
        -v $MODEL_WEIGHT_PATH:/models \
